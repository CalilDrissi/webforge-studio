...
if (Webforge === undefined) var Webforge = {};

Webforge.defaultComponent = "_base";
Webforge.preservePropertySections = true;
//icon = use component icon when dragging | html = use component html to create draggable element
Webforge.dragIcon = 'icon';
...
Webforge.dragElementStyle = "background:limegreen;width:100%;height:3px;border:1px solid limegreen;box-shadow:0px 0px 2px 1px rgba(0,0,0,0.14);overflow:hidden;";
Webforge.dragHtml = '<div style="' + Webforge.dragElementStyle + '"></div>';

Webforge.baseUrl =  document.currentScript?document.currentScript.src.replace(/[^\/]*?\.js$/,''):'';
Webforge.imgBaseUrl =  Webforge.baseUrl;

Webforge.ComponentsGroup = {};
Webforge.SectionsGroup = {};
Webforge.BlocksGroup = {};
Webforge.StylesGroup = {};
...
Webforge.Components = {
	...
	renderProperties: function(component, properties, componentsPanelSections, container) {
		let fn = function(component, property) {
			if (property.input) {
				property.input.addEventListener('propertyChange', (event) => {
						element = selectedElement = Webforge.Builder.selectedEl;
						...
						if (property.htmlAttr) {
							...
							if (property.htmlAttr == "class" && property.validValues) {
								...
								oldStyle = window.FrameDocument.getElementById("webforge-styles").textContent;
								element = Webforge.StyleManager.setStyle(element, property.key, value);
							} else if (property.htmlAttr == "innerHTML")  {
								oldValue = element.innerHTML;
								element = Webforge.ContentManager.setHtml(element, value);
							} else if (property.htmlAttr == "outerHTML")  {
								oldValue = element.outerHTML;
								element = Webforge.ContentManager.setHtml(element, value, true);
							} else if (property.htmlAttr == "innerText")  {
								oldValue = element.innerText;
								element = Webforge.ContentManager.setText(element, value);
							} else {
								oldValue = element.getAttribute(property.htmlAttr);
							...
							if (property.htmlAttr == "style") {
								mutation = {
									...
									oldValue: oldStyle, 
									newValue: window.FrameDocument.getElementById("webforge-styles").textContent};
								
								Webforge.Undo.addMutation(mutation);
							} else {
						...
						if (property.child || property.parent) {
							Webforge.Builder.selectNode(selectedElement);
						} else {
							Webforge.Builder.selectNode(element);
						}
		...
		let nodeElement = Webforge.Builder.selectedEl;
		...
		for (let i in properties) {
			...
			if (property.init) {
				...
				if (property.htmlAttr == "style") {
					...
					value = Webforge.StyleManager.getStyle(element, property.key);//getStyle returns declared style
				} else
				if (property.htmlAttr == "innerHTML") {
					value = Webforge.ContentManager.getHtml(element);
				} else if (property.htmlAttr == "outerHTML") {
					value = Webforge.ContentManager.getHtml(element, true);
				} else if (property.htmlAttr == "innerText") {
					value = Webforge.ContentManager.getText(element);
				} else {
			...
			if (property.inputtype == SectionInput) {
				...
				if (Webforge.preservePropertySections && section) {
				...
				let row = generateElements(tmpl('webforge-property', property))[0]; 
	...
	render: function(type, panel = false) {
		...
		if (!(Webforge.preservePropertySections && section)) {
			let template = tmpl("webforge-input-sectioninput", {key:"default", header:component.name});
		...
		if (component.beforeInit) component.beforeInit(Webforge.Builder.selectedEl);

		this.renderProperties(component, component.properties, componentsPanelSections);
		
		if (component.init) component.init(Webforge.Builder.selectedEl);
	}
};	
...
Webforge.WysiwygEditor = {
	...
	editorSetStyle: function (tag, style = {}, toggle = false) {
		let iframeWindow = Webforge.Builder.iframe.contentWindow;
		...
		if (tag == "a") {
			Webforge.Builder.selectNode(element);
			Webforge.Builder.loadNodeComponent(element);
		}
		return element;
	},
	...
	init: function(doc) {
		...
		document.getElementById("font-family").addEventListener("change", function (e) {
				...
				Webforge.FontsManager.addFont(option.dataset.provider, this.value, element);
	...
	edit: function(element) {
		...
		document.getElementById("font-family").value = Webforge.StyleManager.getStyle(element,'font-family');
		document.getElementById("fore-color").value = Webforge.StyleManager.getStyle(element,'color');
		document.getElementById("back-color").value = Webforge.StyleManager.getStyle(element,'background-color');
		element.focus();
	},
	...
	destroy: function(element) {
		...
		node = this.element;
		Webforge.Undo.addMutation({type:'characterData', 
								target: node, 
...
/* controls */    	
	loadControlGroups : function() {	
		...
		componentsList.forEach(function (list, i) {
			...
			for (group in Webforge.ComponentsGroup) {
				...
				components = Webforge.ComponentsGroup[ group ];
				
				for (i in components) {
					const componentType = components[i];
					component = Webforge.Components.get(componentType);
					...
					if (component) {
						...
						if (component.image) {
							...
							item.style.backgroundImage = "url(" + Webforge.imgBaseUrl + component.image + ")"; 			
	...
	loadSectionGroups : function() {	
		...
		sectionsList.forEach(function (list, i) {
			...
			for (group in Webforge.SectionsGroup) {
				list.append(generateElements(
				`<li class="header" data-section="${group}"  data-search="">
				...
				let sections = Webforge.SectionsGroup[ group ];

				for (i in sections) {
					const sectionType = sections[i];
					const section = Webforge.Sections.get(sectionType);
					...
					if (section) {
						...
						if (section.image) {
							...
							let image = ((section.image.indexOf('/') == -1) ? Webforge.imgBaseUrl:'') + section.image;
	...
	loadBlockGroups : function() {	
		...
		blocksList.forEach(function (list, i) {
			...
			for (group in Webforge.BlocksGroup) {
				list.append(generateElements(
				`<li class="header" data-section="${group}"  data-search="">
				...
				blocks = Webforge.BlocksGroup[ group ];

				for (i in blocks) {
					const blockType = blocks[i];
					const block = Webforge.Blocks.get(blockType);
					...
					if (block) {
						...
						if (block.image) {
							...
							let image = ((block.image.indexOf('/') == -1) ? Webforge.imgBaseUrl:'') + block.image;
							/*
							Object.assign(item.style,{
								//backgroundImage: "url(" + image + ")",
								//backgroundRepeat: "no-repeat"
	 ...
	 loadStylesGroups : function() {	
		...
		stylesList.forEach(function (list, i) {
			...
			for (group in Webforge.StylesGroup) {
				list.append(generateElements(
				`<li class="header" data-section="${group}"  data-search="">
				...
				styles = Webforge.StylesGroup[ group ];

				for (i in styles) {
					const styleType = styles[i];
					const style = Webforge.Styles.get(styleType);
					...
					if (style) {
						...
						if (style.image) {
							...
							let image = ((style.image.indexOf('/') == -1) ? Webforge.imgBaseUrl:'') + style.image;
							item.querySelector("img").setAttribute("src", image);
						}
	...
	loadUrl : function(url, callback) {	
		...
		self.initCallback = callback;
		if (Webforge.Builder.iframe.src != url) Webforge.Builder.iframe.src = url;
	},
...
/* iframe */
	_loadIframe : function(url) {	
	    ...
	    return this.documentFrame.addEventListener("load", function() {
				...
				window.FrameDocument.addEventListener("click", function(event) {
					if (Webforge.WysiwygEditor.isActive && event.target.closest("a"))  {
						event.preventDefault();
						return false;
					}
				});
				...
				Webforge.WysiwygEditor.init(window.FrameDocument);
				Webforge.StyleManager.init(window.FrameDocument);
				Webforge.ColorPaletteManager.init(window.FrameDocument);
    ...
    _frameLoaded : function() {
		...
		self.frameHead.append(generateElements('<link data-webforge-helpers href="' + Webforge.baseUrl + '../../css/webforge-editor-helpers.css" rel="stylesheet">')[0]);
		self.frameHtml.setAttribute("data-webforge-editor","");

		self._initHighlight();
		
		window.dispatchEvent(new CustomEvent("webforge.iframe.loaded", {detail: self.frameDoc}));

		document.querySelector(".loading-message").classList.remove("active");
		Webforge.NewSection.init();
		Webforge.StyleList.load(self.frameDoc);
		
		//enable save button only if changes are made
		let setSaveButtonState = function (e) { 
			if (Webforge.Undo.hasChanges()){
		...
		Webforge.Builder.frameBody.addEventListener("webforge.undo.add", setSaveButtonState);		
		Webforge.Builder.frameBody.addEventListener("webforge.undo.restore", setSaveButtonState);		
    },	
	...
	loadNodeComponent:  function(node) {
		const data = Webforge.Components.matchNode(node);
		let component;
		
		if (data) 
			component = data.type;
		else 
			component = Webforge.defaultComponent;
			
		Webforge.component = Webforge.Components.get(component);	
		Webforge.Components.render(component);
	...
	reloadComponent:  function() {
		Webforge.Components.render(this.selectedComponent);
	},
	
	moveNodeUp:  function(node) {
		if (!node) {
			node = Webforge.Builder.selectedEl;
		}
		...
		Webforge.Undo.addMutation({type: 'move', 
								target: node,
								oldParent: oldParent,
								newParent: newParent,
	...
	cloneNode:  function(node) {
		...
		Webforge.Undo.addMutation({type: 'childList', 
	...
	selectNode:  function(node) {
		...
		if (self.texteditEl && (self.selectedEl != node)) {
			Webforge.WysiwygEditor.destroy(self.texteditEl);
		...
		try {
			...
			Webforge.Breadcrumb.loadBreadcrumb(target);
		...
		window.dispatchEvent(new CustomEvent("webforge.Builder.selectNode", {detail: {target}}));
	},
...
/* iframe highlight */    
    _initHighlight: function() {
		...
		let highlightMove = function(event) {
			if (self.highlightEnabled == true && event.target/* && isElement(event.target)*/) {
				...
				}// else //uncomment else to disable parent highlighting when dragging
				{
					...
					if (Webforge.WysiwygEditor.isActive )  {
		...
		let highlightUp = function(event) {
			...
			if (self.isDragging) {
				...
				Webforge.Builder.highlightEnabled = true;
				if (self.iconDrag) self.iconDrag.remove();
				document.getElementById("component-clone")?.remove();

				if (self.dragMoveMutation === false) {				
					if (self.component.dragHtml || Webforge.dragHtml) { //if dragHtml is set for dragging then set real component html
						if (self.component) {
				...
				Webforge.TreeList.loadComponents();
				Webforge.TreeList.selectComponent(node);
				...
				if (self.dragMoveMutation === false) {
					...
					Webforge.Undo.addMutation(self.dragMoveMutation);
		...
		let highlightDbClick = function(event) {
			...
			if (Webforge.Builder.isPreview == false && Webforge.Builder.highlightEnabled) {
				
				if (!Webforge.WysiwygEditor.isActive)  {
					self.selectPadding = 10;
					self.texteditEl = target = event.target;

					Webforge.WysiwygEditor.edit(self.texteditEl);
		...
		let highlightClick = function(event) {
			...
			if (Webforge.Builder.isPreview == false && Webforge.Builder.highlightEnabled){
				if (event.target) {
					...
					Webforge.TreeList.selectComponent(event.target);
					self.loadNodeComponent(event.target);

					if (Webforge.component.resizable) {
						document.getElementById("select-box").classList.add("resizable");
                      	self.resizeMode = Webforge.component.resizeMode;
					} else {
	...
	_initBox: function() {
		...
		document.getElementById("drag-btn").addEventListener("mousedown", function(event) {
			//self.dragElement = self.selectedEl.setAttribute("style",Webforge.dragElementStyle);
			if (event.which == 1) {//left click
				...
				if (self.designerMode) {
					...
					self.dragElement = generateElements(Webforge.dragHtml)[0];
				}
		...
		document.getElementById("save-reusable-btn").addEventListener("click", function(event) {
			...
			if (name) {
				Webforge.Builder.saveElement(node, type, name);
			}
		...
		document.getElementById("edit-code-btn").addEventListener("click", function(event) {
			let value = Webforge.Builder.selectedEl.innerHTML;

			Webforge.ModalCodeEditor.show();
			Webforge.ModalCodeEditor.setValue(value);
		...
		let onSave = function(event) {
			Webforge.Builder.selectedEl.innerHTML = event.detail;
			
			const node = Webforge.Builder.selectedEl;
			Webforge.Undo.addMutation({type:'characterData', 
				target: node, 
		...
		window.addEventListener("webforge.ModalCodeEditor.save", onSave); 
		
		document.getElementById("translate-code-btn")?.addEventListener("click", function(event) {
			let selectedEl = Webforge.Builder.selectedEl;
			...
			window.removeEventListener("webforge.ModalCodeEditor.save", onSave); 
			window.addEventListener("webforge.ModalCodeEditor.save", onSave); 
		...
		document.getElementById("delete-btn").addEventListener("click", function(event) {
			...
			Webforge.TreeList.loadComponents();
			Webforge.SectionList.loadSections();
		...
		function addSectionComponent(component, after = true) {
			...
			if (["section", "footer", "header", "nav"].includes(tagName)) {
				if (!element) {
				element = Webforge.Builder.frameBody.querySelector(':scope > ' + tagName + ':last-of-type') || Webforge.Builder.frameBody.querySelector(':scope > section:last-of-type') || Webforge.Builder.frameBody.querySelector(':scope > header:last-of-type') || Webforge.Builder.frameBody.querySelector(':scope > main') || Webforge.Builder.frameBody.querySelector(':scope > footer:last-of-type');
				} else {
			...
			if (element) {
				...
				let newsection = Webforge.Builder.frameBody.querySelector('newsection')
				if (newsection) {
					newsection.before(node);
				} else {
					Webforge.Builder.frameBody.append(node);
				}
			}
		...
		addSectionBox.addEventListener("click", function(event) {
			...
			if (element) {
				...
				switch (itemType) {
					case 'component':
						html = Webforge.Components.get(element.dataset.type);
					break;					
					case 'block':
						html = Webforge.Blocks.get(element.dataset.type);
					break;					
					case 'section':
						html = Webforge.Sections.get(element.dataset.type);
					break;
				}
...
/* drag and drop */
		...
		document.addEventListener("mousedown", function(event) {
			...
			if (element && event.which == 1) {//left click
				...
				if (self.dragType == "component") {
					self.component = Webforge.Components.get(element.dataset.type);
				}
				else if (self.dragType == "section") {
					self.component = Webforge.Sections.get(element.dataset.type);
				}
				else if (self.dragType == "block") {
					self.component = Webforge.Blocks.get(element.dataset.type);
				}
				
				if (self.component.dragHtml) {
					html = self.component.dragHtml;
				} else if (Webforge.dragHtml) { 
					html = Webforge.dragHtml;
				} else {
	...
	removeHelpers: function (html, keepHelperAttributes = false) {
		...
		html = html.replace(/<[^>]+?data-webforge-helpers.+?>/gi, "");
		html = html.replace(/<[^>]+?webforge-new-section.+?>.+?<\/newsection>/gims, "");
		//attributes
		if (!keepHelperAttributes) {
			html = html.replace(/\s*data-webforge-\w+(=["'].*?["'])?\s*/gi, "");
		}
		
		html = html.replaceAll("webforge-hidden", "").replaceAll("data-webforge-editor", "");
		return html;
	},
	...
	getHtml: function(keepHelperAttributes = true, filter = true) {
		...
		if (filter) {
			window.dispatchEvent(new CustomEvent("webforge.getHtml.before", {detail: doc}));
		}
		...
		Webforge.FontsManager.cleanUnusedFonts();

		html += doc.documentElement.outerHTML;
		html = this.removeHelpers(html, keepHelperAttributes);

		if (filter) {
			 window.dispatchEvent(new CustomEvent("webforge.getHtml.after", {detail: doc}));
			 window.dispatchEvent(new CustomEvent("webforge.getHtml.filter", {detail: html}));
		}
	...
	saveElement: function(element, type, name, callback) {
		if (type == 'section') {
			Webforge.Sections.add('reusable/'+ name, {
				name,
				image: "img/logo-small.png",
				html: element.outerHTML});
			
			if (Webforge.SectionsGroup["Reusable"] === undefined) {
				Webforge.SectionsGroup["Reusable"] = [];
			}
			
			Webforge.SectionsGroup["Reusable"].push('reusable/'+ name);
			Webforge.Builder.loadSectionGroups();
		} else {
			Webforge.Blocks.add('reusable/'+ name, {
				name,
				image: "img/logo-small.png",
				html: element.outerHTML});
			
			if (Webforge.BlocksGroup["Reusable"] === undefined) {
				Webforge.BlocksGroup["Reusable"] = [];
			}
			
			Webforge.BlocksGroup["Reusable"].push('reusable/'+ name);
			Webforge.Builder.loadBlockGroups();
		}
	...
	saveAjax: function(data, saveUrl, callback, error ) {
		if (!data["file"]) {
			data["file"]  = Webforge.FileManager.getCurrentFileName();
		}
		...
		.then((data) => {
			...
			Webforge.Undo.reset();
			document.querySelectorAll("#top-panel .save-btn").forEach(e => e.setAttribute("disabled", "true"));
			window.dispatchEvent(new CustomEvent("webforge.Builder.saveAjax", {
				detail: data,
			}));
		})
		.catch((err) => {
...
Webforge.CodeEditor = {
	...
	init: function(doc) {
		this.textarea = document.querySelector("#webforge-code-editor textarea");
		this.textarea.value = Webforge.Builder.getHtml();

		this.textarea.addEventListener("keyup", e => {
			delay(() => Webforge.Builder.setHtml(this.value), 1000);
		});
...
Webforge.CssEditor = {
	...
	init: function(doc) {
		...
		this.textarea.value = Webforge.StyleManager.getCss();
		let self = this;
		/*
		document.querySelectorAll('[href="#csscode-tab"]').forEach( t => t.addEventListener("click", e => {
			self.textarea.value = Webforge.StyleManager.getCss();
		}));
		*/
		this.textarea.addEventListener("keyup", e => {
			delay(() => Webforge.StyleManager.setCss(self.textarea.value), 1000);
		});
	},
...
Webforge.Gui = {
	...
	init: function() {
		document.querySelectorAll("[data-webforge-action]").forEach(function (el,i) {
			const on = el.dataset.webforgeOn ?? "click";
			el.addEventListener(on, Webforge.Gui[el.dataset.webforgeAction]);
		});
	...
	shortcuts: function() {
			...
			if (e.ctrlKey) {
				switch (e.key) {
					case 's':
						...
						let url = btn.dataset.webforgeUrl;
		...
		window.addEventListener('webforge.iframe.loaded', () => {
			Webforge.Builder.frameBody.addEventListener('keydown', handleShortcuts);		
		});
	...
	undo : function () {
		if (Webforge.WysiwygEditor.isActive) {
			Webforge.WysiwygEditor.undo();
		} else {
			Webforge.Undo.undo();
		}
		Webforge.Builder.selectNode();
	},
	
	redo : function () {
		if (Webforge.WysiwygEditor.isActive) {
			Webforge.WysiwygEditor.redo();
		} else {
			Webforge.Undo.redo();
		}
		Webforge.Builder.selectNode();
	...
	save : function () {
		document.getElementById('textarea-modal textarea').value = Webforge.Builder.getHtml();
	...
	saveAjax : function (event, saveUrl = null, saveBtn = null) {
		...
		saveUrl = saveUrl ?? this.dataset.webforgeUrl;
		let file = Webforge.FileManager.getPageData('file');
		...
		return Webforge.Builder.saveAjax({file}, saveUrl, (data) => {
	...
	download : function () {
		const filename = /[^\/]+$/.exec(Webforge.Builder.iframe.src)[0];
		const uriContent = "data:application/octet-stream,"  + encodeURIComponent(Webforge.Builder.getHtml());
	...
	viewport : function () {
		...
		if (Webforge.StyleManager.currentBreakpoint == breakpoint) {
			Webforge.StyleManager.currentBreakpoint = "none";
			breakpoint = null;
		}
		
		Webforge.Builder.documentFrame.style.width = "";
		Webforge.Builder.documentFrame.style.height = "";
		
		if (breakpoint) {
			document.getElementById("canvas").classList.add("responsive");
			document.getElementById("iframe-wrapper").style.width = Webforge.StyleManager.breakpoints[breakpoint].replace(".98", "");
			Webforge.StyleManager.currentBreakpoint = breakpoint;
			this.classList.add("active");
		}
	},
	
	toggleEditor : function () {
		document.getElementById("webforge-builder").classList.toggle("bottom-panel-expand");
		...
		Webforge.CodeEditor.toggle();
		Webforge.CssEditor.toggle();
	},
	
	toggleHidden : function () {
		Webforge.Builder.frameBody.classList.toggle("webforge-hidden");
	},

	toggleEditorJsExecute : function () {
		Webforge.Builder.runJsOnSetHtml = this.checked;
	},
	
	preview : function () {
		(Webforge.Builder.isPreview == true)?Webforge.Builder.isPreview = false:Webforge.Builder.isPreview = true;
		document.getElementById("iframe-layer").classList.toggle("d-none");
		document.getElementById("webforge-builder").classList.toggle("preview");
	},
	...
	newPage : function () {
		...
		let submitForm = function(e) {
			...
			if (data['file']) {
				...
				//data['url']  = Webforge.themeBaseUrl + data['url'];
			}

			e.preventDefault();

			return Webforge.Builder.saveAjax(data, this.action, function (savedData) {
					...
					let page = Webforge.FileManager.addPage(data.name, data);
					Webforge.FileManager.loadPage(data.name);
					Webforge.FileManager.scrollToPage(page);
					bsModal.hide();
			});
		};
	...
	setDesignerMode : function () {
		...
		Webforge.Builder.setDesignerMode(designerMode);
	},
	...
	toggleFileManager: function () {
		Webforge.Gui.togglePanel("#filemanager", "--builder-filemanager-height");
	},
	
	toggleLeftColumn: function () {
		Webforge.Gui.togglePanel("#left-panel", "--builder-left-panel-width");
	},	
	
	toggleRightColumn: function (rightColumnEnabled = null) {
		rightColumnEnabled = Webforge.Gui.togglePanel("#right-panel", "--builder-right-panel-width");

		document.getElementById("webforge-builder").classList.toggle("no-right-panel");
		document.querySelector(".component-properties-tab").classList.toggle("d-none");
		
		Webforge.Components.componentPropertiesElement = (rightColumnEnabled ? "#right-panel" :"#left-panel #properties") + " .component-properties";
	...
	treeListRight: function () {
		let treeList = document.getElementById("tree-list");
		let btnIcon = document.querySelector("[data-webforge-action='treeListRight'] i");
	...
	treeListPanel: function () {
		...
		let btnIcon = document.querySelector("[data-webforge-action='treeListPanel'] i");
	...
	setState: function () {
		Webforge.StyleManager.setState(this.value);
		Webforge.Builder.reloadComponent();
	}	
}

Webforge.StyleManager = {
	...
	init: function(doc) {
		if (doc) {
			...
			for (let i = 0; i < doc.styleSheets.length; i++) {
					...
					if (_style.ownerNode.id && _style.ownerNode.id == "webforge-styles") {
			...
			if (!style) {
				style = generateElements('<style id="webforge-styles"></style>')[0];
	...
	getSelectorForElement: function(element) {
		...
		while (currentElement.parentElement) {
			...
			let classSelector = Array.from(currentElement.classList).map(function (className) {
					if (Webforge.Builder.ignoreClasses.indexOf(className) == -1) {
						return "." + className;
					}
				}).join("");
	...
	setStyle: function(element, styleProp, value) {
		...
		window.dispatchEvent(new CustomEvent("webforge.StyleManager.setStyle", {detail: {element, styleProp, value}}));
	...
	generateCss: function (media) {
		...
		this.cssContainer = this.doc.getElementById("webforge-styles");
		...
		if (node.hasChildNodes()) {
			for (let j = 0; j < node.childNodes.length; j++) {
				...
				if (child && child["attributes"] != undefined && 
					(matchChild = Webforge.Components.matchNode(child))) {
		...
		for (i in tree) {
			...
			if (tree[i].children.length > 0) {
				li = generateElements('<li data-component="' + node.name + '">\
								<label for="id' + id + '" style="background-image:url(' + Webforge.imgBaseUrl + node.image + ')">\
...
Webforge.SectionList = {
	...
	init: function(allowedComponents = {}) {
		...
		document.querySelector(this.selector).addEventListener("click", function (e) {
			...
			if (element) {
				...
				if (node) {
					...
					//node.click();
					Webforge.Builder.selectNode(node);
					Webforge.Builder.loadNodeComponent(node);
				}
			}
		});
		...
		document.querySelector(this.selector).addEventListener("click", ".up-btn", function (e) {
			...
			let node = section._node;
			Webforge.Builder.moveNodeUp(node);
			Webforge.Builder.moveNodeUp(section);
		...
		document.querySelector(this.selector).addEventListener("click", ".down-btn", function (e) {
			let section = e.target.closest(".section-item");
			let node = section._node;
			Webforge.Builder.moveNodeDown(node);
			Webforge.Builder.moveNodeDown(section);
		...
		*/
		...
		document.querySelector(".sections-list").addEventListener("click", function (e) {
			...
			if (element) {
				...
				let section = Webforge.Sections.get(item.dataset.type);
				let node = generateElements(section.html)[0];
				let sectionType = node.tagName.toLowerCase();
				let afterSection = Webforge.Builder.frameBody.querySelector(":scope > " + sectionType + ":last-of-type");
				
				if (afterSection) {
					afterSection.after(node);
				} else {
					if (sectionType == "nav") {
						afterSection = Webforge.Builder.frameBody.querySelector(":scope > nav:first,> header:last-of-type");		
						...
						afterSection = Webforge.Builder.frameBody.querySelector("body > footer:last-of-type");		
				...
				Webforge.Builder.loadNodeComponent(node);
				/*
				Webforge.Builder.frameHtml.animate({
	...
	addSection: function(data) {
		let section = generateElements(tmpl("webforge-section", data))[0];
...
Webforge.StyleList = {
	...
	applyStyle: function(style) {
		let doc =  window.FrameDocument;
		let themeStyle = doc.getElementById("webforge-theme-style");
		...
		if (!themeStyle) {
			...
			themeStyle.id = "webforge-theme-style";
	...
	load: function(doc) {
		let themeStyle = window.FrameDocument.getElementById("webforge-theme-style");
		if (themeStyle) {
			let style = Webforge.Styles.getByStyle(themeStyle.getAttribute("href"));
			if (style) {
	...
	init: function() {
		...
		document.querySelector(".styles-list").addEventListener("click", function (e) {
			...
			if (element) {
				let style = Webforge.Styles.get(element.dataset.type);
...
Webforge.FileManager = {
	...
	init: function(allowedComponents = {}) {
		...
		this.tree.addEventListener("click", function (e) {
			...
			if (element) {
				Webforge.FileManager.deletePage(element.closest("li"), e);
				e.stopImmediatePropagation();
				e.preventDefault();
				return false;
			}
		...
		this.tree.addEventListener("click", function (e) {
			...
			if (element) {
				Webforge.FileManager.renamePage(element.closest("li"), e, false);
				e.stopImmediatePropagation();
				e.preventDefault();
				return false;
			}
		...
		this.tree.addEventListener("click", function (e) {
			...
			if (element) {
				Webforge.FileManager.renamePage(element.closest("li"), e, true);
				e.stopImmediatePropagation();
				e.preventDefault();
				return false;
			}
		...
		this.tree.addEventListener("click", function (e) {
			...
			if (element) {
				...
				if (page) Webforge.FileManager.loadPage(page, allowedComponents);
				e.preventDefault();
				return false;			
			}
		});
	...
	deletePage: function(element, e) {
		...
		if (confirm(`Are you sure you want to delete "${page.file}"template?`)) {
			...
			window.dispatchEvent(new CustomEvent("webforge.FileManager.deletePage", {
				detail
			}));
	...
	renamePage: function(element, e, duplicate = false) {
		...
		if (newfile) {
			...
			window.dispatchEvent(new CustomEvent("webforge.FileManager.renamePage", {
				detail
			}));
			...
			if (detail.page) {
				...
				.then((data) => {
						...
						if (duplicate) {
							...
							data["file"] = newfile;
							data["title"] = newName;
							Webforge.FileManager.addPage(baseName, data);
						} else {
	...
	addPage: function(name, data, afterPage = false, append = true) {
		...
		window.dispatchEvent(new CustomEvent("webforge.FileManager.addPage", {
			detail: [name, data],
		}));
		...
		if (data.folder) {
			if ((data.folder && data.folder != "/") && !(folder = folder.querySelector('li[data-folder="' + data.folder + '"]')))  {
				...
				folder = generateElements(tmpl("webforge-filemanager-folder", data))[0];
		...
		let page = generateElements(tmpl("webforge-filemanager-page", data))[0];
	...
	addComponent: function(name, url, title, page) {
		document.querySelector("[data-page='" + page + "'] > ol", this.tree).append(
			tmpl("webforge-filemanager-component", {name:name, url:url, title:title}));
	},
	...
	loadPage: function(name, allowedComponents = false, disableCache = true, loadComponents = false) {
		...
		window.dispatchEvent(new CustomEvent("webforge.FileManager.loadPage", {
			detail: self.pages[name],
		}));

		if (url) {
			Webforge.Builder.loadUrl(url + (disableCache ? (url.indexOf('?') > -1 ? '&r=':'?r=') + Math.random():''), 
				function () { 
					if (loadComponents) { Webforge.FileManager.loadComponents(allowedComponents); }
					Webforge.SectionList.loadSections(allowedComponents); 
					Webforge.TreeList.loadComponents(); 
					Webforge.StyleManager.init(); 
				});
		}
	},
...
Webforge.Breadcrumb = {
	...
	addElement: function(data, element) {
		let li = generateElements(tmpl("webforge-breadcrumb-navigaton-item", data))[0];
		li._node = element;			
	...
	loadBreadcrumb: function(element) {
		...
		while (currentElement.parentElement) {
			let elementType = Webforge.Builder._getElementType(currentElement);
...
Webforge.FontsManager = {
	...
	addFontList: function(provider, groupName, fontList) {
		...
		let list = Webforge.Components.getProperty("_base", "font-family");
		if (list) {
			list.onChange = function (node,value, input, component) {
				let option = input.options[input.selectedIndex];
				Webforge.FontsManager.addFont(option.dataset.provider, value, node);
				return node;
			};
			...
			Webforge.Components.updateProperty("_base", "font-family", {data:list.data});
	...
	cleanUnusedFonts: function (){
		for (i in this.activeFonts) {
			...
			if (elementFont.element) {
				if (Webforge.StyleManager.getStyle(elementFont.element,'font-family').replaceAll('"','') != elementFont.fontFamily) {
...
Webforge.ColorPaletteManager = {
	...
	init: function(document) {
		Webforge.Components.render("config/bootstrap", "#configuration .component-properties");
		
		//apply current theme color palette
		//let colors = Webforge.ColorPaletteManager.getType("color");
		...
		for (const name in colors) {
			...
			if (color[0] == "#" && color.length == 7) {//add only valid hex color values 7 char long
				...
				Webforge.ColorPalette.add(color, color);
			}
		}
	},
...
Webforge.NewSection = {
	container:null,
	template:`<newsection id="webforge-new-section">
			  <div class="container">
				<div class="row">
				  <div class="col">
	...
	init: function() {
		...
		this.container.onclick = function(event) {
			if (event.target.id == 'new-blank-section') {
				Webforge.Gui.newSection(null,true);
			} else {
				if (event.target.id == 'new-section') {
					Webforge.Gui.newSection(null,false);
				}
			}
		};
	},	
	...
	insert: function() {
		let position = 'before';
		let lastSection = Webforge.Builder.frameBody.querySelector(':scope > footer:last-of-type');
		if (!lastSection) {
			position = 'after';
			lastSection = Webforge.Builder.frameBody.querySelector(':scope > main:last-of-type') || 
			Webforge.Builder.frameBody.querySelector(':scope > section:last-of-type');
		}
		...
		if (lastSection) {
			...
			Webforge.Builder.frameBody.append(this.container)
		}
