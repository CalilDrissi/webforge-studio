...
window.addEventListener("webforge.getHtml.before", function(event) {
	let doc = event.detail;
	doc.querySelectorAll("[data-aos]").forEach(e => e.classList.remove("aos-animate", "aos-init"));
});

window.addEventListener("webforge.getHtml.after", function(event) {
	let doc = event.detail;
	doc.querySelectorAll("[data-aos]").forEach(e => e.classList.add("aos-animate", "aos-init"));
});

//ignore aos classes for styles
Webforge.Builder.ignoreClasses = Webforge.Builder.ignoreClasses.concat(["aos-init", "aos-animate"]);
...
Webforge.Components.extend("_base", "_base", ComponentBaseAnimateScroll);
