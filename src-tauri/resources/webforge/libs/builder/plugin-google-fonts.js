...
GoogleFontsManager = {
	...
	updateFontList: function () {
		let googleFontsLink = Webforge.Builder.frameHead.querySelector("#google-fonts-link");
		...
		if (!googleFontsLink) {
			...
			Webforge.Builder.frameHead.append(googleFontsLink);
		}
...
Webforge.FontsManager.addProvider("google", GoogleFontsManager);
	
let googleFonts = {};
let googlefontNames = [];
//load google fonts list and update wyswyg font selector and style tab font-family list
fetch(Webforge.baseUrl + "../../resources/google-fonts.json")
.then((response) => {
	if (!response.ok) { throw new Error(response) }
	return response.json()
})
.then((data) => {
	Webforge.FontsManager.addFontList("google", "Google Fonts", data);
})
.catch(error => {
