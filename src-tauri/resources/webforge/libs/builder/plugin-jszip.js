...
function () {
    ...
    let html = Webforge.Builder.frameHtml;
    ...
    Promise.all(promises).then((data) => {
        let html = Webforge.Builder.getHtml();
        ...
        zip.file(Webforge.FileManager.getCurrentFileName() ?? "index.html", html);
        zip.generateAsync({type:"blob"})
        .then(function(content) {
            saveAs(content, Webforge.FileManager.getPageData("title") ?? Webforge.FileManager.getCurrentPage());
        });
    }).catch((error) => {
        console.log(error)
  })
