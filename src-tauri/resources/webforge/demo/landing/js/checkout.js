function refreshCart(parameters, element, update = false) {
	webforgeTheme.Cart.module = 'checkout/checkout';
	webforgeTheme.Cart.component_id = 1;
	//action, parameters, element, selector, callback
	if (!update) {
		update = ['.cart-summary', '.container > .notifications'];
	}
	webforgeTheme.Cart.ajax('', parameters, element, update);
	//cart-summary
}
...
document.addEventListener('click', function (e) {
	...
	if (element) {
		...
		if (element.classList.contains("btn-remove-coupon")) {
			...
			webforgeTheme.Cart.removeCoupon({coupon}, element, updateElements);
			container.remove();
		} else {
			let coupon = document.querySelector("[name='coupon']").value;
			webforgeTheme.Cart.coupon({coupon}, element, updateElements);
		}
		e.preventDefault();
	}
});
