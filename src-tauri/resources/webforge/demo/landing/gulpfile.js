...
async function screenshots(type = "sections", dirs = []) {
	...
	*/ 
	...
	for (i in sections) {
		...
		let html = `<html data-webforge-editor><head><base href="../../"><link href="/css/style.bundle.css" rel="stylesheet"><link href="/css/screenshots.css" rel="stylesheet"><style>[data-reveal], [data-section] > * {
    opacity: 1;
...
'section':{
	'group':`
	Webforge.SectionsGroup["{{group}}"] = [{{groupList}}];
`,
	'section':
`Webforge.Sections.add("{{id}}", {
    name: "{{name}}",
    image: Webforge.themeBaseUrl + "/{{image}}",
    html: \`{{html}}\`
});`
},
'block': {
	'group':`
	Webforge.BlocksGroup['{{group}}'] = [{{groupList}}];
`,
	'section':
`Webforge.Blocks.add("{{id}}", {
    name: "{{name}}",
    image: Webforge.themeBaseUrl + "/{{image}}",
    html: \`{{html}}\`
});`
...
//generate sections for Webforge
function sections(type = "section", dirs = []) {
	...
	for (i in dir) {
	  ...
	  let filePath = `${sectionsDir}/${fileName}`;
		...
		let id;
		let html;
		let sectionsHtml = `<html data-webforge-editor><head><base src="../"><link href="../css/style.bundle.css" rel="stylesheet"><link href="../css/screenshots.css" rel="stylesheet"><style>[data-reveal], [data-section] > * {
    opacity: 1;
    transform: translate(0);
